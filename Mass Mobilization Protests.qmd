...
KNN seems to have the highest recall. Since we care about capturing as many positive labels as possible in this case, KNN might be a good choice in real world scenarios. However, it has lower accuracy and roc_auc compared to other models, which leads me to believe that it does a worse job in distinguishing between positive and negative labels. Also, while it's possible to indirectly understand feature importance through KNN, it's not as intuitive as tree or regression based model. For these reasons, I'd select Random Forest as a model of choice.
