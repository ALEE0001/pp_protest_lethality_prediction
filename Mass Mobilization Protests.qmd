...
Random Forest Model arguably performs a ted bit better, but Logistic Regression may be a better choice in real life scenario since it is more explanable and also scales better with bias towards Positive Labels. Given that consequences are human death, we care more about catching "Lethality" Labels as much as possible, even if that means lowering the total accuracy of the model.
